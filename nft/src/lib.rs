...
impl Contract {
    ...
    pub fn update_metadata(&mut self, token_id: TokenId, token_metadata: TokenMetadata) -> bool {
        self.tokens.token_metadata_by_id
            .as_mut()
            .and_then(|by_id| by_id.insert(&token_id, Some(token_metadata).as_ref().unwrap()));
        true
    }
}
